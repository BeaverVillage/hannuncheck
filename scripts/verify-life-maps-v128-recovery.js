...
pages.forEach((p) => { assert(exists(p), `${p} missing`); assert(read(p).includes('20260623-v131-ev106-emergency-mobile-life-ui'), `${p} v128 query missing`); });
jsFiles.forEach((p) => { const t = read(p); assert(t.includes("v131-ev106-emergency-mobile-life-ui"), `${p} v128 runtime missing`); assert(t.includes('useCurrentLocation'), `${p} current location handler missing`); assert(t.includes('handlePlaceSearch'), `${p} place search handler missing`); assert(t.includes('life-reference-marker'), `${p} reference marker missing`); assert(t.includes('keywordSearch'), `${p} Kakao keyword search missing`); assert(t.includes('addressSearch'), `${p} Kakao address search missing`); });
