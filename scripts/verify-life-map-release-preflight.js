...
for (const tool of tools) {
  ...
  assert(html.includes(VERSION), `${tool.name}: html cache-busting query is v130-ev106-life-map-mobile-fix`);
  assert(js.includes(`const VERSION = '${RUNTIME}'`), `${tool.name}: runtime version is v130-ev106-life-map-mobile-fix`);
