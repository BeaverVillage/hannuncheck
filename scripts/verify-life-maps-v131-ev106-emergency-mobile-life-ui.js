...
for (const [name, htmlFile, jsFile] of tools) {
  ...
  assert(js.includes("applyMobileState('collapsed')") && js.includes("applyMobileState('expanded')"), `${name}: sheet snap states missing`);
}
...
assert(css.includes('position: absolute') && css.includes('bottom: 18px'), 'css: current location button must be absolute inside map');
...
assert(css.includes('minmax(220px, 1fr)') && css.includes('minmax(122px, 132px)'), 'css: wifi/toilet toolbar compact columns missing');
...
assert(!evHtml.includes('v131-ev106-emergency-mobile-life-ui'), 'ev charger html must not load life-map v130 assets');
