...
for (const file of fs.readdirSync(path.join(root, 'tools')).filter((name) => name.endsWith('.html'))) {
  ...
  if (html.includes('/assets/js/app.js')) {
    assert(html.includes('/assets/js/app.js?v=20260623-v128-recovered-location-search'), `${file} uses v128-recovered-location-search app.js`);
  }
}
assert(index.includes('/assets/js/app.js?v=20260623-v128-recovered-location-search'), 'index uses v128-recovered-location-search app.js');
