...
for (const tool of tools) {
  ...
  assert(html.includes(VERSION), `${tool.name}: html cache-busting query is v128-recovered-location-search`);
  assert(js.includes(`const VERSION = '${RUNTIME}'`), `${tool.name}: runtime version is v128-recovered-location-search`);
