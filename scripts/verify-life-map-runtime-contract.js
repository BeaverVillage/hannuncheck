...
const SHARED_VERSION = '20260623-v119-life-maps-triple-stability';
const tools = [
  {
    name: '낚시터',
...
for (const tool of tools) {
  ...
  assert(html.includes(SHARED_VERSION), `${tool.name}: html asset query is v119`);
  assert(js.includes(`const VERSION = '${tool.runtimeVersion}'`), `${tool.name}: js runtime version is v119`);
