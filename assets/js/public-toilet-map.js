(() => {
  const root = document.querySelector('[data-public-toilet-map-tool]');
  if (!root) return;
  ...
  const elements = {
    form: root.querySelector('#toilet-form'),
    region: root.querySelector('#toilet-region'),
    district: root.querySelector('#toilet-district'),
    keyword: root.querySelector('#toilet-keyword'),
    category: root.querySelector('#toilet-category'),
    openType: root.querySelector('#toilet-open-type'),
    sort: root.querySelector('#toilet-sort'),
    openAlways: root.querySelector('#toilet-open-always'),
    disabled: root.querySelector('#toilet-disabled'),
    baby: root.querySelector('#toilet-baby'),
    bell: root.querySelector('#toilet-bell'),
    ...
    listSummary: root.querySelector('#toilet-list-summary'),
    listSubtitle: root.querySelector('#toilet-list-subtitle'),
    resultList: root.querySelector('#toilet-result-list'),
    mobileResults: root.querySelector('#toilet-mobile-results'),
    mobileTitle: root.querySelector('#toilet-mobile-sheet-title'),
    mobileSubtitle: root.querySelector('#toilet-mobile-sheet-subtitle'),
    mobileToggle: root.querySelector('#toilet-mobile-list-toggle'),
    mobileSheet: root.querySelector('#toilet-mobile-bottom-sheet'),
    mobileMapButton: root.querySelector('#toilet-mobile-sheet-map-button'),
    countCard: root.querySelector('#toilet-count-card'),
    openCard: root.querySelector('#toilet-open-card'),
    disabledCard: root.querySelector('#toilet-disabled-card'),
    safetyCard: root.querySelector('#toilet-safety-card'),
    sortButtons: Array.from(root.querySelectorAll('[data-toilet-sort]')),
    map: root.querySelector('#toilet-map'),
    markers: root.querySelector('#toilet-map-markers'),
    mapNotice: root.querySelector('#toilet-map-notice'),
    mapTitle: root.querySelector('#toilet-map-title'),
    selectedCard: root.querySelector('#toilet-selected-card'),
    mapToolbarSearch: root.querySelector('#toilet-map-toolbar-search'),
    mapKeyword: root.querySelector('#toilet-map-keyword'),
    mapRegion: root.querySelector('#toilet-map-region'),
    mapDistrict: root.querySelector('#toilet-map-district'),
  };
  ...
  const hasOpenAlways = (item) => detailsOf(item).openType === '상시개방';
  const hasDisabled = (item) => item?.__hasDisabled === true || detailsOf(item).hasDisabledToilet === true;
  ...
  const prepareRuntimeItem = (item = {}) => {
    ...
    const searchText = item.searchText || [
      ...
      details.manager,
      details.ownerType,
      details.emergencyBellLocation,
      details.babyChangingLocation,
    ].join(' ');
    Object.defineProperties(item, {
      __prepared: { value: true, enumerable: false },
      __searchKey: { value: normalizeSearch(searchText), enumerable: false },
      __category: { value: normalize(item.category), enumerable: false },
      __openType: { value: normalize(details.openType), enumerable: false },
      __hasDisabled: { value: details.hasDisabledToilet === true, enumerable: false },
      __hasBaby: { value: details.hasBabyChanging === true, enumerable: false },
      __hasBell: { value: details.hasEmergencyBell === true, enumerable: false },
      __hasCctv: { value: details.hasCctv === true, enumerable: false },
    });
    return item;
  };
  ...
  const matchesFilters = (item) => {
    ...
    const category = normalize(elements.category?.value || '');
    const openType = normalize(elements.openType?.value || '');
    if (keyword && !item.__searchKey.includes(keyword)) return false;
    if (category && item.__category !== category) return false;
    if (openType && item.__openType !== openType) return false;
    if (elements.openAlways?.checked && !hasOpenAlways(item)) return false;
    if (elements.disabled?.checked && !hasDisabled(item)) return false;
    if (elements.baby?.checked && !hasBaby(item)) return false;
    if (elements.bell?.checked && !hasBell(item)) return false;
    if (elements.cctv?.checked && !hasCctv(item)) return false;
    return true;
  };
  ...
  const sortItems = (items) => {
    ...
    const disabledScore = (item) => hasDisabled(item) ? 1 : 0;
    const babyScore = (item) => hasBaby(item) ? 1 : 0;
    const safetyScore = (item) => (hasBell(item) ? 1 : 0) + (hasCctv(item) ? 1 : 0);
    return [...items].sort((a, b) => {
      if (sort === 'name') return byName(a, b);
      if (sort === 'open') return openScore(b) - openScore(a) || byDistance(a, b) || byName(a, b);
  ...
  const loadDistrict = async (regionKey, districtKey) => {
    ...
    state.loading = true;
    setStatus(`${regionLabel(requestedRegion)} 공중화장실 데이터를 준비하는 중입니다.`);
    ...
    try {
      ...
      state.currentDistrict = requestedDistrict;
      setStatus(`${regionLabel(requestedRegion)} ${districtLabel(requestedRegion, requestedDistrict)} 공중화장실 데이터를 불러오는 중입니다.`);
      ...
      if (!district) {
        ...
        setStatus(`${regionLabel(requestedRegion)} ${districtLabel(requestedRegion, requestedDistrict)} 공중화장실 ${state.rawItems.length.toLocaleString('ko-KR')}곳을 표시합니다.`);
      }
    } catch (error) {
      if (requestId !== state.requestId) return;
      state.rawItems = [];
      ...
      state.mapAutoFitPending = true;
      setStatus('공중화장실 로컬 캐시를 불러오지 못했습니다. 캐시 생성 후 다시 확인해 주세요.', 'error');
      setMapNotice('캐시 확인 필요', 'assets/data/life/public-toilets/index.json 파일을 확인해 주세요.');
    } finally {
  ...
  const populateDynamicFilters = (items) => {
    fillSelect(elements.category, collectValues(items.map((item) => item.category)), '전체');
    fillSelect(elements.openType, collectValues(items.map((item) => item.details?.openType)), '전체');
  };
  ...
  const syncLocationControls = (regionKey, districtKey) => {
    ...
    if (elements.mapTitle) elements.mapTitle.textContent = `${regionLabel(regionKey)} ${districtLabel(regionKey, districtKey)} 공중화장실`;
  };
  ...
  const renderSummary = () => {
    ...
    const openCount = state.items.filter(hasOpenAlways).length;
    const disabledCount = state.items.filter(hasDisabled).length;
    const safetyCount = state.items.filter(hasSafety).length;
    const place = locationLabel();
    if (elements.listTitle) elements.listTitle.textContent = `${place} 공중화장실 목록`;
    if (elements.listSummary) elements.listSummary.textContent = `${total.toLocaleString('ko-KR')}곳`;
    if (elements.listSubtitle) elements.listSubtitle.textContent = total ? `목록은 최대 ${Math.min(total, MAX_LIST)}곳, 지도는 최대 ${Math.min(total, MAX_MARKERS)}개 마커를 표시합니다.` : '조건에 맞는 공중화장실이 없습니다.';
    if (elements.mobileTitle) elements.mobileTitle.textContent = `${place} 공중화장실 목록`;
    ...
    updateSummaryCard(elements.openCard, '상시개방', `${openCount.toLocaleString('ko-KR')}곳`, '24시간·상시 참고');
    updateSummaryCard(elements.disabledCard, '장애인 화장실', `${disabledCount.toLocaleString('ko-KR')}곳`, '시설 수 기준');
    updateSummaryCard(elements.safetyCard, '안전설비', `${safetyCount.toLocaleString('ko-KR')}곳`, '비상벨 또는 CCTV');
    if (!state.loading) {
      const suffix = state.referencePoint || state.geo ? '현재 위치 기준 거리도 함께 표시합니다.' : '선택 지역 중심 기준으로 가까운 순을 참고합니다.';
      setStatus(total ? `${place} 공중화장실 ${total.toLocaleString('ko-KR')}곳을 표시합니다. ${suffix}` : '조건에 맞는 공중화장실이 없습니다.', total ? 'info' : 'warning');
    }
  };
  ...
  const renderList = (target, items, options = {}) => {
    if (!target) return;
    if (!items.length) {
      target.innerHTML = '<div class="empty-state"><strong>조건에 맞는 공중화장실이 없습니다.</strong><p>지역, 시군구, 검색어 또는 필터를 조정해 다시 확인해 주세요.</p></div>';
      return;
    }
  ...
  const featureBadges = (item) => {
    const labels = [];
    if (hasDisabled(item)) labels.push('장애인 가능');
    if (hasBaby(item)) labels.push('기저귀대');
    if (hasBell(item)) labels.push('비상벨');
    if (hasCctv(item)) labels.push('CCTV');
    return labels.length ? labels.join(' · ') : '편의시설 확인 필요';
  };
  ...
  const makeMarkerElement = (item, index) => {
    ...
    marker.type = 'button';
    marker.className = `life-marker toilet-marker ${state.selectedId === item.id ? 'selected' : ''}`;
  ...
  const renderFallbackMap = (items) => {
    ...
    const validItems = items.filter(isValidPoint).slice(0, MAX_MARKERS);
    if (!validItems.length) {
      setMapNotice('표시할 좌표 없음', '조건에 맞는 공중화장실 좌표가 없습니다.');
      return;
    }
    ...
    validItems.forEach((item, index) => {
      ...
      wrapper.type = 'button';
      wrapper.className = `life-marker toilet-marker life-fallback-marker ${state.selectedId === item.id ? 'selected' : ''}`;
  ...
  const initKakaoMap = async () => {
    ...
    try {
      ...
      setMapNotice('카카오 지도', '공중화장실 위치를 지도에 표시합니다.');
      renderMap(state.items);
    } catch (error) {
  ...
  const bindEvents = () => {
    ...
    elements.category?.addEventListener('change', () => applyFilters({ resetSelection: true }));
    elements.openType?.addEventListener('change', () => applyFilters({ resetSelection: true }));
    [elements.openAlways, elements.disabled, elements.baby, elements.bell, elements.cctv].forEach((element) => element?.addEventListener('change', () => applyFilters({ resetSelection: true })));
