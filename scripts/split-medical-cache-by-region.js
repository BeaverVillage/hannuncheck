 ...
 *
 * 입력 파일이 있으면 아래 단일 캐시를 분할하고 원본 단일 파일은 제거합니다.
...
const VERSION = 'v103-medical-cache-split';
...
const writePayload = (file, oldPayload, entries, region, type, sourceFile) => {
  const meta = {
    ...(oldPayload.meta || {}),
    version: 'v103-medical-regional-split',
  ...
  writeJson(file, { version: 'v103-medical-regional-split', meta, entries });
};
...
const partitionIfNeeded = (file) => {
  ...
  for (let i = 0; i < items.length; i += chunkSize) {
    ...
    writeJson(partPath, {
      version: old.version || 'v103-medical-regional-split',
      meta: { ...(old.meta || {}), partitionOf: path.basename(file), partitionIndex: parts.length + 1, totalEntries: Object.keys(partEntries).length },
      entries: partEntries,
    });
  ...
  writeJson(file, {
    version: old.version || 'v103-medical-regional-split',
    meta: { ...(old.meta || {}), partitioned: true, totalEntries: items.length, partCount: parts.length, note: `${old.meta?.note || ''} 파일 크기 제한 대응을 위해 파티션으로 분할된 매니페스트입니다.`.trim() },
    parts,
    entries: {},
  });
};
...
const splitNightCache = (sourceFile, outputDir, type) => {
  ...
  const manifest = { version: 'v103-medical-regional-split', meta: { type, sourceFile, totalEntries: Object.keys(old.entries || {}).length }, regions: {} };
  REGIONS.forEach(([, full]) => {
    const entries = buckets.get(full) || {};
    const file = path.join(output, `${full}.json`);
    writePayload(file, old, entries, full, type, sourceFile);
    partitionIfNeeded(file);
    manifest.regions[full] = { file: `${full}.json`, count: Object.keys(entries).length };
  });
  if (buckets.has('기타')) {
    const file = path.join(output, '기타.json');
    writePayload(file, old, buckets.get('기타'), '기타', type, sourceFile);
    partitionIfNeeded(file);
    manifest.regions['기타'] = { file: '기타.json', count: Object.keys(buckets.get('기타')).length };
  }
...
const splitKakaoCache = () => {
  ...
  const manifest = { version: 'v103-medical-regional-split', meta: { type: 'medical-kakao-place', sourceFile, totalEntries: Object.keys(old.entries || {}).length }, emergency: { file: 'emergency.json', count: Object.keys(emergency).length }, hospital: {}, pharmacy: {} };
  for (const type of ['hospital', 'pharmacy']) {
    REGIONS.forEach(([, full]) => {
      const entries = buckets[type].get(full) || {};
      const file = path.join(output, type, `${full}.json`);
      writePayload(file, old, entries, full, `medical-kakao-${type}`, sourceFile);
      partitionIfNeeded(file);
      manifest[type][full] = { file: `${type}/${full}.json`, count: Object.keys(entries).length };
    });
  }
  writeJson(path.join(output, 'index.json'), manifest);
  fs.unlinkSync(source);
