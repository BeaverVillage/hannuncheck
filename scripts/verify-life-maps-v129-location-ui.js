...
pages.forEach((p) => {
  const html = read(p);
  assert(html.includes('20260623-v131-ev106-emergency-mobile-life-ui'), `${p} v129 query missing`);
});
scripts.forEach((p) => {
  const js = read(p);
  assert(js.includes('v131-ev106-emergency-mobile-life-ui'), `${p} v129 version missing`);
