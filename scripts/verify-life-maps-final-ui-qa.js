...
for (const tool of tools) {
  ...
  assert(html.includes(version), `${tool.name}: v125-life-maps-ui-complete cache busting applied`);
  assert(js.includes(`const VERSION = '${runtime}'`), `${tool.name}: v125-life-maps-ui-complete runtime version applied`);
...
assert(appVersionMisses.length === 0, `all tool app.js references use v125-life-maps-ui-complete (${appVersionMisses.join(', ') || 'ok'})`);
assert(read('index.html').includes(`/assets/js/app.js?v=${version}`), 'index app.js reference uses v125-life-maps-ui-complete');
if (errors.length) {
