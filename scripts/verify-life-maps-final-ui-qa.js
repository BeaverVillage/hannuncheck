...
for (const tool of tools) {
  ...
  assert(html.includes(version), `${tool.name}: v126-life-maps-ui-polish-final cache busting applied`);
  assert(js.includes(`const VERSION = '${runtime}'`), `${tool.name}: v126-life-maps-ui-polish-final runtime version applied`);
...
assert(wifiHtml.includes('와이파이 이름 있는 곳만'), 'wifi: user-friendly filter label');
assert(wifiHtml.includes('와이파이 이름 있음') || wifiHtml.includes('와이파이 이름 제공'), 'wifi: user-friendly summary label');
...
assert(appVersionMisses.length === 0, `all tool app.js references use v126-life-maps-ui-polish-final (${appVersionMisses.join(', ') || 'ok'})`);
assert(read('index.html').includes(`/assets/js/app.js?v=${version}`), 'index app.js reference uses v126-life-maps-ui-polish-final');
if (errors.length) {
