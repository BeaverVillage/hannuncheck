...
for (const tool of tools) {
  ...
  assert(html.includes(version), `${tool.name}: v124-life-maps-final-ui-qa cache busting applied`);
  assert(js.includes(`const VERSION = '${runtime}'`), `${tool.name}: v124-life-maps-final-ui-qa runtime version applied`);
...
assert(wifiHtml.includes('와이파이 이름 제공'), 'wifi: user-friendly summary label');
...
assert(appVersionMisses.length === 0, `all tool app.js references use v124-life-maps-final-ui-qa (${appVersionMisses.join(', ') || 'ok'})`);
assert(read('index.html').includes(`/assets/js/app.js?v=${version}`), 'index app.js reference uses v124-life-maps-final-ui-qa');
if (errors.length) {
